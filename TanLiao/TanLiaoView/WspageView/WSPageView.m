        ...
        case NewPagedFlowViewOrientationHorizontal:{
            ...
            for (int i = self.visibleRange.location; i < self.visibleRange.location + _visibleRange.length; i++) {
                ...
                if (delta < _pageSize.width) {
                     ...
                     float biLi = cell.bounds.size.width/(VIEW_WIDTH-150);
                    ...
                    cell.mainImageView.frame = CGRectMake(cell.bounds.origin.x, cell.bounds.origin.y, cell.bounds.size.width, cell.bounds.size.height);
                    cell.coverView.frame = CGRectMake(cell.bounds.origin.x, cell.bounds.origin.y, cell.bounds.size.width, cell.bounds.size.width*1.5);
                    
                    cell.titleLable.font = [UIFont systemFontOfSize:18*cell.bounds.size.width/(VIEW_WIDTH-150)];
                    
                   
                    
                     cell.titleLable.frame = CGRectMake(cell.titleLable.frame.origin.x, cell.titleLable.frame.origin.y*biLi, cell.bounds.size.width, cell.bounds.size.width*20/(VIEW_WIDTH-150));
                    
                    CGSize oneLineSize = [TanLiao_Common setSize:@"新人报道" withCGSize:CGSizeMake(VIEW_WIDTH-(146*USERCC*2), VIEW_HEIGHT) withFontSize:18];
                    
                    cell.imageViewLeft.frame =  CGRectMake(((cell.bounds.size.width-oneLineSize.width*biLi)/2-10*biLi-16*biLi), 2, 16*biLi, 16*biLi);
                    
                    cell.imageViewRight.frame = CGRectMake((cell.bounds.size.width-oneLineSize.width*biLi)/2+oneLineSize.width*biLi+10*biLi, 2, 16*biLi, 16*biLi);
                } else {
        ...
        case NewPagedFlowViewOrientationVertical:{
            ...
            for (int i = self.visibleRange.location; i < self.visibleRange.location + _visibleRange.length; i++) {
                ...
                if (delta < _pageSize.height) {
                    ...
                    cell.frame = UIEdgeInsetsInsetRect(originCellFrame, UIEdgeInsetsMake(inset, inset, inset, inset));
                    cell.mainImageView.frame = CGRectMake(cell.bounds.origin.x+5*BILI, cell.bounds.origin.y, cell.bounds.size.width-10*BILI, cell.bounds.size.height);
                    cell.coverView.frame = CGRectMake(cell.bounds.origin.x, cell.bounds.origin.y, cell.bounds.size.width, cell.bounds.size.width*1.5);
                    ...
                    cell.titleLable.frame = CGRectMake(cell.titleLable.frame.origin.x, cell.titleLable.frame.origin.y, cell.bounds.size.width, cell.bounds.size.width*20/(VIEW_WIDTH-150));
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if ([_delegate respondsToSelector:@selector(didScrollToPage:inFlowView:)] && _currentPageIndex != pageIndex) {
        [_delegate didScrollToPage:_scrollView.contentOffset.x inFlowView:self];
    }
    _currentPageIndex = pageIndex;
}
-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    NSLog(@"%d####,%d",(int)scrollView.contentOffset.x,[UIScreen mainScreen].bounds.size.width);
  //  scrollView.contentOffset.x/(VIEW_WIDTH-150);
     [_delegate didScrollToPage:scrollView.contentOffset.x inFlowView:self];
}
