...
+(NSString *)getParamStr1
{
    NSUserDefaults * jinbiDefaults = [NSUserDefaults standardUserDefaults];
    return [TanLiao_Common getobjectForKey:[jinbiDefaults objectForKey:@"iosCheckParameter1"]];
    
    
}
+(NSString *)getParamStr2
{
    NSUserDefaults * fenZhongDefaults = [NSUserDefaults standardUserDefaults];
    return [TanLiao_Common getobjectForKey:[fenZhongDefaults objectForKey:@"iosCheckParameter2"]];
}
...
+ (BOOL)validateUserName:(NSString*)number {
    BOOL res = YES;
    if(![TanLiao_Common isValidString:number])
        return NO;
...
{
    CGSize oneLineSize = [TanLiao_Common setSize:message withCGSize:CGSizeMake(VIEW_WIDTH, VIEW_HEIGHT) withFontSize:12];
...
//{
//    UIView * loadingBottomView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, VIEW_WIDTH, VIEW_HEIGHT)];
//    loadingBottomView.backgroundColor = [UIColor clearColor];
//    loadingBottomView.tag = -1314120;
...
//    
//    UIImageView * loadingImageView = [[UIImageView alloc]initWithFrame:CGRectMake((VIEW_WIDTH-60)/2,(VIEW_HEIGHT-60)/2, 60, 60)];
...
+(void)uploadGuanZhuIdList:(NSMutableArray *)guanZhuList
{
    ...
    if ([guanZhuList isKindOfClass:[NSArray class]]&&guanZhuList!=nil) {
        ...
        NSUserDefaults * defaults = [NSUserDefaults standardUserDefaults];
        [defaults setObject:[TanLiao_Common removeNullFromArray:guanZhuList] forKey:@"userGuanZhuList"];
...
+ (NSMutableArray *)removeNullFromArray:(NSArray *)arr
{
    ...
    for (int i = 0; i < arr.count; i++) {
        ...
        if ([value isKindOfClass:NSDictionary.class]) {
            [marr addObject:[TanLiao_Common removeNullFromDictionary:(NSDictionary *)value]];
        }
...
+ (NSMutableDictionary *)removeNullFromDictionary:(NSDictionary *)dic
{
    ...
    for (NSString *strKey in dic.allKeys) {
        ...
        // 删除NSArray中的NSNull，再保存进字典
            ...
            mdic[strKey] = [TanLiao_Common removeNullFromArray:(NSArray *)value];
        }
...
{
    NSData *imageData = [TanLiao_Common dataWithScreenshotInPNGFormat];
...
+(UIImage * )erweima :(NSString *)dingDanHao
{
    ...
    return [TanLiao_Common createNonInterpolatedUIImageFormCIImage:outputImage withSize:100.0];
