...
-(void)initRenwWuView
{
    ...
    TanLiaoCustomImageView * tipImageView = [[TanLiaoCustomImageView alloc] initWithFrame:CGRectMake((VIEW_WIDTH-490*BILI/2)/2, 130*BILI/2, 490*BILI/2, 690*BILI/2)];
...
-(void)getShareImageSuccess:(NSDictionary *)info
{
    ...
    if ([@"1" isEqualToString:[TanLiao_Common getCurrentUserAnchorType]]&&[@"1" isEqualToString:[info objectForKey:@"shareStatus"]])
    {
...
-(void)initShareView:(NSDictionary *)info
{
    ...
    tipLable.text = [NSString stringWithFormat:@"分享可获得%d金币",shareCoin.intValue/100];
...
-(void)huanYiPiSuccess:(NSArray *)list
{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW,  NSEC_PER_SEC), dispatch_get_main_queue(), ^{
        ...
        if(self.mainTableView.frame.origin.y+self.mainTableView.frame.size.height<self.mainScrollView.frame.size.height) {
            [self.mainScrollView setContentSize:CGSizeMake(VIEW_WIDTH, self.mainScrollView.frame.size.height+50*BILI)];
...
//    WSPageView *pageView = [[WSPageView alloc]initWithFrame:CGRectMake(0, self.headerScrollView.frame.origin.y+self.headerScrollView.frame.size.height-8*BILI, VIEW_WIDTH, 136*BILI)];
...
//    UIView * topThreeBottomView = [[UIView alloc] initWithFrame:CGRectMake(0, self.headerScrollView.frame.origin.y+self.headerScrollView.frame.size.height, VIEW_WIDTH, 328*BILI/2+10*BILI)];
//    topThreeBottomView.backgroundColor = [UIColor blackColor];
//    topThreeBottomView.alpha = 0.05;
//    [self.mainScrollView addSubview:topThreeBottomView];
//    
//    self.topThreeBotomView = [[UIView alloc] initWithFrame:CGRectMake(0, self.headerScrollView.frame.origin.y+self.headerScrollView.frame.size.height+5*BILI, VIEW_WIDTH, 328*BILI/2)];
//    self.topThreeBotomView.backgroundColor = [UIColor whiteColor];
//    self.topThreeBotomView.clipsToBounds = YES;
//    [self.mainScrollView addSubview:self.topThreeBotomView];
//    
//    
//
//    [self getTopThreeSourceData];
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section
{
    if (section ==0) {
         ...
         return (VIEW_WIDTH/375*157)*80/157+15;
    }
    else
    {
        return 0;
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    if (section ==0) {
        ...
        UIView * sectionView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, VIEW_WIDTH, VIEW_WIDTH/375*90)];
        
        UIImageView * imageView = [[UIImageView alloc] initWithFrame:CGRectMake((VIEW_WIDTH-VIEW_WIDTH/375*157)/2, 5, VIEW_WIDTH/375*157, (VIEW_WIDTH/375*157)*80/157)];
        imageView.image = [UIImage imageNamed:@"pic_hot"];
