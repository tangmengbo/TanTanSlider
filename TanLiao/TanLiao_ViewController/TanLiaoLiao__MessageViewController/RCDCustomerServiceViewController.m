...
- (void)viewDidLoad {
  ...
  [[RCIMClient sharedRCIMClient] getHumanEvaluateCustomerServiceConfig:^(NSDictionary *evaConfig) {
    ...
    self.fanKuiButton = [[UIButton alloc] initWithFrame:CGRectMake(VIEW_WIDTH-108*BILI/2-20*BILI, VIEW_HEIGHT-80*BILI-SafeAreaBottomHeight-30*BILI, 108*BILI/2, 142*BILI/2)];
    [self.fanKuiButton addTarget:self action:@selector(fanKuiButtonClick) forControlEvents:UIControlEventTouchUpInside];
    [self.fanKuiButton setBackgroundImage:[UIImage imageNamed:@"xiaoxi_btn_fankui"] forState:UIControlStateNormal];
    [self.view addSubview:self.fanKuiButton];

    
}
-(void)fanKuiButtonClick
{
    TanLiao_FeedBackViewController * vc = [[TanLiao_FeedBackViewController alloc] init];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    {
        NSLog(@"self.moveView.y = %f", self.chatSessionInputBarControl.frame.origin.y);
        if (self.chatSessionInputBarControl.frame.origin.y>580) {
            
            self.fanKuiButton.frame = CGRectMake(self.fanKuiButton.frame.origin.x, VIEW_HEIGHT-80*BILI-SafeAreaBottomHeight-30*BILI, self.fanKuiButton.frame.size.width, self.fanKuiButton.frame.size.height);
        }
        else
        {
            if (SafeAreaTopHeight==20) {
                
                self.fanKuiButton.frame = CGRectMake(self.fanKuiButton.frame.origin.x, VIEW_HEIGHT-80*BILI-SafeAreaBottomHeight-30*BILI-250*BILI, self.fanKuiButton.frame.size.width, self.fanKuiButton.frame.size.height);
                
            }
            else
            {
                self.fanKuiButton.frame = CGRectMake(self.fanKuiButton.frame.origin.x, VIEW_HEIGHT-80*BILI-SafeAreaBottomHeight-30*BILI-320*BILI, self.fanKuiButton.frame.size.width, self.fanKuiButton.frame.size.height);
