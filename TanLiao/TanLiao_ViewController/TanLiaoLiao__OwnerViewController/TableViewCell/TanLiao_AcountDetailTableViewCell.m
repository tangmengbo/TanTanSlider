...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self)
    {
        self.tipImageView = [[TanLiaoCustomImageView alloc] initWithFrame:CGRectMake(12*BILI,11*BILI , 30*BILI, 30*BILI)];
...
-(void)initData:(NSDictionary *)info
{
    ...
    else if([@"23" isEqualToString:[info objectForKey:@"trans_type"]])
    {
        //用户浏览小视频扣费
        self.tipImageView.image = [UIImage imageNamed:@"icon_zd_video"];
        self.titleLable.text = @"小视频扣费";
        
    }
    else if([@"24" isEqualToString:[info objectForKey:@"trans_type"]])
    {
        //主播小视频收入
        self.tipImageView.image = [UIImage imageNamed:@"icon_zd_cz"];
        self.titleLable.text = @"小视频扣费";
