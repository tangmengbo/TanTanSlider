...
-(void)levelTimer
{
    ...
    if (luYinTime>=120) {
        ...
        [TanLiao_Common showToastView:@"录音最长120秒" view:self.view];
...
-(void)stopRecordVoice
{
    if(luYinTime<15)
    {
        [TanLiao_Common showToastView:@"录音时长不能少于15秒,请重新录制" view:self.view];
        [self.recordAudioButton setImage:[UIImage imageNamed:@"audio_luzhi_kaishi"] forState:UIControlStateNormal];
        self.tipMesLable.text = @"点击开始录音";
        self.timeLable.text = @"00:00";
        [self stopRecordAndTimer];
        ...
        {
            UILabel * EtaanUzttv = [[UILabel alloc]initWithFrame:CGRectMake(49,12,61,22)];
            EtaanUzttv.layer.cornerRadius =7;
...
- (void)initDataMglfzzVvknykVC:(NSDictionary *)info
{
    ...
    UILabel * HdylyLylwy = [[UILabel alloc]initWithFrame:CGRectMake(26,19,29,25)];
    HdylyLylwy.layer.cornerRadius =8;
    [viewArray addObject:HdylyLylwy];
}
/*
