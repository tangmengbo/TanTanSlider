...
- (instancetype)initWithCallee:(NSString *)callee{
    ...
    if (self) {
        ...
        self.callInfo.caller = [info objectForKey:@"userId"];
...
- (void)dismiss:(void (^)(void))completion{
    ...
    self.navigationController.navigationBarHidden = YES;
    if ([@"yes" isEqualToString:self.alsoPopRoot]) {
               ...
               // [self playSenderRing];
                ...
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delayTime * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    ...
                    if (wself && room && room.count == 1) {
                        ...
                        {
                            [self guDuanQianJieKou:@"9"];//主播超时未接
                        }
                        else
                        {
                             [self guDuanQianJieKou:@"8"];//主播超时未接
                        }
                        wself.chatRoom = nil;
...
- (void)onResponse:(UInt64)callID from:(NSString *)callee accepted:(BOOL)accepted{
    if (self.callInfo.callID == callID) {
        if (!accepted) {
            ...
            if([@"1" isEqualToString:[TanLiao_Common getCurrentUserAnchorType]])
            {
                [self guDuanQianJieKou:@"4"];//主播拒接
            }
            else
            {
                [self guDuanQianJieKou:@"3"];//用户拒接
            }
            [self playHangUpRing];
            __weak typeof(self) wself = self;
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(3.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
...
- (void)onCallDisconnected:(UInt64)callID withError:(NSError *)error
{
    ...
    if([@"1" isEqualToString:[TanLiao_Common getCurrentUserAnchorType]])
    {
       [self guDuanQianJieKou:@"7"];//主播异常挂断
    }
...
- (void)fillUserSetting:(NIMNetCallOption *)option
{
   ...
   // option.videoCrop  = [[NTESBundleSetting sharedConfig] videochatVideoCrop];
    ...
    option.videoAdaptiveStrategy = NIMAVChatVideoAdaptiveStrategySmooth;//流畅度优
...
{
    NIMNetCallVideoProcessorParam *videoProcessorParam = [NIMNetCallVideoProcessorParam alloc];//设置美颜参数
    videoProcessorParam.filterType = NIMNetCallFilterTypeZiran;
    videoProcessorParam.contrastIntensity = 1;
    videoProcessorParam.smoothIntensity = 0.7;
    ...
    /*
    param.videoHandler = ^(CMSampleBufferRef sampleBuffer) {
...
//    }
//    //视频界面镜像反转
//    _bufferDisplayer.transform = CATransform3DMakeRotation(M_PI, 0, 1, 0);
...
- (void)playHangUpRing{
    [self.player stop];
    NSString * soudString  = [[NSBundle mainBundle] pathForResource:@"avchat_peer_reject" ofType:@"mp3"];
...
- (void)playOnCallRing{
    ...
    [self.player stop];
    NSString * soudString  = [[NSBundle mainBundle] pathForResource:@"avchat_peer_busy" ofType:@"mp3"];
    if ([soudString isKindOfClass:[NSString class]]) {
        NSURL *url = [NSURL fileURLWithPath:soudString];
        self.player = [[AVAudioPlayer alloc] initWithContentsOfURL:url error:nil];
        [self.player play];
...
- (void)playTimeoutRing{
    ...
    [self.player stop];
    NSString * soudString  = [[NSBundle mainBundle] pathForResource:@"avchat_no_response" ofType:@"mp3"];
    if ([soudString isKindOfClass:[NSString class]]) {
        NSURL *url = [NSURL fileURLWithPath:soudString];
        self.player = [[AVAudioPlayer alloc] initWithContentsOfURL:url error:nil];
        [self.player play];
...
- (void)playReceiverRing{
    ...
    NSString * soudString  = [[NSBundle mainBundle] pathForResource:@"wx" ofType:@"mp3"];
    if ([soudString isKindOfClass:[NSString class]]) {
        NSURL *url = [NSURL fileURLWithPath:soudString];
        self.player = [[AVAudioPlayer alloc] initWithContentsOfURL:url error:nil];
...
-(void)guDuanQianJieKou:(NSString *)type
{
        if (!hangUpTiJiaoSuccessStatus) {
        hangUpTiJiaoSuccessStatus = YES;
        NSUserDefaults * defaults = [NSUserDefaults standardUserDefaults];
