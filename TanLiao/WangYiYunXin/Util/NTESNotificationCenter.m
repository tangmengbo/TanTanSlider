...
- (void)onReceive:(UInt64)callID from:(NSString *)caller type:(NIMNetCallMediaType)type message:(NSString *)extendMessage{
    ...
    SJTabBarController * rootBar = delegate.rootBar;
    int index = (int)rootBar.selectedIndex;
    UINavigationController * nav = [rootBar.viewControllers objectAtIndex:index];
