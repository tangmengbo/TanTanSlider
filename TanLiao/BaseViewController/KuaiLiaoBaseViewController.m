...
- (void)viewDidLoad {
    ...
    self.baseCloudClient = [KuaiLiaoCloudClient getInstance];
...
-(void)initLaiDianView:(NSDictionary *)info
{
    ...
    KuaiLiaoCustomImageView * headerImageView = [[KuaiLiaoCustomImageView alloc] initWithFrame:CGRectMake(12*BILI, 7*BILI, 36*BILI, 36*BILI)];
    ...
    CGSize size = [Common setSize:[info objectForKey:@"nick"] withCGSize: CGSizeMake(VIEW_WIDTH, VIEW_HEIGHT) withFontSize:15*BILI];
...
-(void)jieShouButton
{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW,  NSEC_PER_SEC), dispatch_get_main_queue(), ^{
       ...
       // [self tingZhiHuJiao];
        ...
        {
            [Common showToastView:@"余额不足请充值" view:self.view];
...
-(void)initPushUserView
{
    ...
    if (array.count>0) {
        ...
        for (int i=0; i<array.count; i++) {
            ...
            KuaiLiaoCustomImageView * headerImageView = [[KuaiLiaoCustomImageView alloc] initWithFrame:CGRectMake(80*BILI, 16*BILI, 90*BILI, 90*BILI)];
